    ...
    inline static void UpdateOled()
    {
        ...
        if (enteredPage)
        {
            ...
            case Page::GAIN:
                ...
                break;
            case Page::SPEED:
                mpaland::sprintf(cstr, "%s%.2fx", cLR, looper.GetReadRate(currentLooper));
                break;
            case Page::START:
            {
    ...
    inline static void ProcessEvent(const UiEventQueue::Event &e)
    {
        ...
        case UiEventQueue::Event::EventType::encoderTurned:
            if (enteredPage)
            {
                ...
                case Page::SPEED:
                {
                    float currentSpeedMult{looper.GetReadRate(currentLooper)};
                    ...
                    looper.SetReadRate(looper.IsDualMode() ? currentLooper : StereoLooper::BOTH, currentSpeedMult);
                    break;
                }
                case Page::START:
                {
